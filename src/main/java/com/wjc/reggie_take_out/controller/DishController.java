...
public class DishController {
    ...
    public R<String> save(@RequestBody DishDto dishDto){
        ...
        //为了防止脏数据，方法一，清理所有菜品的缓存数据
//        Set keys = redisTemplate.keys("dish_*");
//        redisTemplate.delete(keys);

        //方法二，精确清理，只清理update的分类下面的菜品缓存数据
...
//        for(DishDto record : records){ //这里的问题记录一下吧。。。想一想再
//            Long categoryId = record.getCategoryId(); // 分类的id
//            Category category = categoryService.getById(categoryId);
//            String categoryName = category.getName();
...
//        redisTemplate.delete(keys);
    ...
    /**
     * 新增套餐界面新增菜品，可以查询已有的菜品，根据条件查询对应的菜品数据
     * @param dish
     * @return
     */
//    @GetMapping("/list")
...
//        queryWrapper.eq(Dish::getStatus, 1); //添加条件，只查询状态为1起售的菜品
//
//        //添加排序条件
...
//    }
    ...
    public R<List<DishDto>> list(Dish dish){
        ...
        //动态构造key
        String key = "dish_" + dish.getCategoryId() + "_" + dish.getStatus(); //dish_1397844291040167938_1
    ...
    public R<String> changeSale(@PathVariable("status") Integer status, @RequestParam List<Long> ids){
        ...
        dishService.changeSale(status, ids);
